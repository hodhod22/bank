...
router.get("/zarinpal/callback", async (req, res) => {
  const { Authority, Status, userId } = req.query;
  const sender = await User.findById(userId);

  console.log("user id", userId);
  if (Status === "OK") {
    try {
      let amount = 10000; // Replace with the actual amount you used in the original payment request
      ...
      /*  await User.findByIdAndUpdate(
        userId,
        { $inc: { [`balance.IRR`]: parseFloat(verifiedAmount) } },
        { new: true }
      ); */
      if (response.status === 100) {
        res.json({
        ...
        sender.balance["IRR"] += amount;
        console.log("gholi", sender.balance["IRR"]);
        await sender.save();
      } else {
        res.status(400).json({
          message: "Payment verification failed.",
